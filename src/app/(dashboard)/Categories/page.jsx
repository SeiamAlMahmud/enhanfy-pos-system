...
export default function CategoryList() {
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/categories/post-categories`,
        {
          method: 'POST',
  ...
  const exportPDF = () => {
    ...
    products.forEach((product, index) => {
      doc.text(
        `${index + 1}. ${product.category}, Created On: ${product.created_on}, Status: ${product.status}`,
        20,
        20 + index * 10
      );
    });
  ...
  async function fetchCategories() {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/categories/get-categories`
        ...
        <div className="fixed inset-0 flex items-center justify-center  transition-all duration-700 ease-in-out ">
          <div className="bg-white w-[90%] md:w-[40%] lg:w-[30%] text-center p-6 rounded shadow-lg relative">
            <h2 className="text-lg mb-4 text-black">
              {modalType === 'edit'
                ? `Edit Category: ${selectedCategory.category}`
                : 'Are you sure?'}
            </h2>
            {modalType === 'edit' ? (
              <form
                onSubmit={handleSaveChanges}
                className=" w-full bg-white rounded text-start text-black"
              >
                <button
                  onClick={handleCloseModal}
              ...
              <div className="text-black ">
                ...
                <p>
                  <strong>ID:</strong> {selectedCategory.category_slug}
                </p>
                <p>
                  <strong>ID:</strong> {selectedCategory.status}
                </p>
                <p>
        ...
        <div className="fixed inset-0 flex items-center justify-center  transition-all duration-700 ease-in-out">
          <div className="bg-white w-[90%] md:w-[40%] lg:w-[30%] text-center p-6 rounded shadow-lg relative">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={closeImportProductModal}
                className="btn btn-sm btn-circle btn-ghost absolute right-3 top-2"
              >
                ✕
              </button>
