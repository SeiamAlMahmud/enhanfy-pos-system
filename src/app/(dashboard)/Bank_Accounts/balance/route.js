...
export async function POST(req) {
  try {
    ...
    if (isNaN(transferAmount) || transferAmount <= 0) {
      return new Response(
        JSON.stringify({ error: 'Invalid transfer amount provided' }),
        { status: 400 }
      );
    }
    ...
    if (fromAccountResult.rowCount === 0) {
      await query('ROLLBACK');
      return new Response(
        JSON.stringify({ error: 'Source account not found' }),
        { status: 404 }
      );
    }

    const fromAccountBalance = parseFloat(
      fromAccountResult.rows[0].current_balance
    );
    ...
    if (fromAccountBalance < transferAmount) {
      await query('ROLLBACK');
      return new Response(
        JSON.stringify({ error: 'Insufficient balance in the source account' }),
        { status: 400 }
      );
    }
    ...
    const toAccountBalance = parseFloat(
      toAccountResult.rows[0].current_balance
    );
