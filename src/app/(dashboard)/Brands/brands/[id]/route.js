...
export async function PATCH(request, { params }) {
  try {
    ...
    const selectQuery =
      "SELECT brands FROM categories WHERE EXISTS (SELECT 1 FROM unnest(brands) AS b WHERE b::jsonb->>'id' = $1)";
    ...
    const brandsArray = selectResult.rows[0].brands?.map((brand) =>
      JSON.parse(brand)
    );
    ...
    const updatedBrandsTextArray = updatedBrandsArray?.map((brand) =>
      JSON.stringify(brand)
    );

    // Update the brands column
    const updateQuery =
      "UPDATE categories SET brands = $2 WHERE EXISTS (SELECT 1 FROM unnest(brands) AS b WHERE b::jsonb->>'id' = $1)";
...
export async function DELETE(request, { params }) {
  try {
    ...
    const updatedBrandsArray = brandsArray.filter(
