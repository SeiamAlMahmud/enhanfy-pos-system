    ...
    const fetchPurchases = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/purchase/get-purchase`
        ); // Update the endpoint based on your API setup
  ...
  const applyFilters = () => {
    ...
    if (filter.startDate) {
      filteredPurchases = filteredPurchases.filter(
        (purchase) =>
          new Date(purchase.purchase_date) >= new Date(filter.startDate)
      );
    }
    ...
    if (filter.endDate) {
      filteredPurchases = filteredPurchases.filter(
        (purchase) =>
          new Date(purchase.purchase_date) <= new Date(filter.endDate)
      );
    }
