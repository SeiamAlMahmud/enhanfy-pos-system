...
export async function GET() {
  try {
    ...
    return new Response(JSON.stringify({ damages: result.rows }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error fetching damages:', error);
    return new Response(JSON.stringify({ error: 'Failed to fetch damages' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
...
export async function POST(request) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to add damage' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
...
export async function DELETE(request) {
  try {
    ...
    if (!id) {
      return new Response(JSON.stringify({ error: 'Damage ID is required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (result.rowCount === 0) {
      return new Response(JSON.stringify({ error: 'Damage not found' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete damage' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
