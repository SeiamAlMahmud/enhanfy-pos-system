...
const SalesReturnList = () => {
    ...
    const fetchReturnedSales = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/returns/get-returns`
  ...
  const uniqueCustomers = [
    ...new Set(returnedSales.map((item) => item.selected_customer)),
  ];
  const uniqueStatuses = [...new Set(returnedSales.map((item) => item.status))];
  const uniquePaymentStatuses = [
    ...new Set(returnedSales.map((item) => item.status)),
  ];
  ...
  const filteredSalesReturns = returnedSales.filter((sale) => {
    ...
    return (
      productMatch &&
      (selectedCustomer === '' ||
        sale.selected_customer === selectedCustomer) &&
      (selectedStatus === '' || sale.status === selectedStatus) &&
      (selectedPaymentStatus === '' ||
        sale.paymentStatus === selectedPaymentStatus)
    );
  });
                  ...
                  sale.products.map((product, productIndex) => (
                    ...
                    >
                      ...
                      <td className="p-2 border">
                        $
                        {parseFloat(
                          sale.total_payable - sale.amount_paid
                        ).toFixed(2)}
                      </td>
