...
export default function Home() {
  ...
  const todaySold = todaySales.reduce(
    (acc, sale) => acc + parseFloat(sale?.total || 0),
    0
  );
  const todaySoldCost = todaySales.reduce((acc, sale) => {
    return (
      ...
      (sale?.products?.reduce(
        (productAcc, product) =>
          productAcc + (product?.purchase_cost || 0) * (product?.quantity || 0),
        0
      ) || 0)
    );
  }, 0);
  ...
  const monthSold = monthSales.reduce(
    (acc, sale) => acc + parseFloat(sale?.total || 0),
    0
  );
  const monthPurchase =
    purchases
      ...
      .reduce((acc, purchase) => acc + parseFloat(purchase?.total || 0), 0) ||
    0;
    ...
    <div className="dark:text-white lg:p-8 font-nunito text-sm">
      <main className="ml-1/5 flex-grow md:mt-[5%] mt-[20%]">
        ...
        <div className="lg:mt-5 bg-white dark:bg-[#141432] text-gray-900 dark:text-gray-100 p-4">
          ...
          <h2 className="dark:text-white text-white text-md mb-4 mt-8">
            Today Summary
          </h2>
          <div className="grid grid-cols-1 text-white sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatsCard
              color="#8A2BE2"
              title="Today Sold - Purchase Cost"
              value={`Tk ${todaySoldCost}`}
            />
            <StatsCard
              color="#FF6347"
              title="Today Expense"
              value={`Tk ${todayExpense}`}
            />
            <StatsCard
              color="#32CD32"
              title="Today Sell Profit"
              value={`Tk ${todayProfit}`}
            />
          </div>

          <h2 className="dark:text-white text-md mb-4 mt-8">
            Current Month Summary
          </h2>
          <div className="grid grid-cols-1 text-white sm:grid-cols-2 md:grid-cols-4 gap-4">
            <StatsCard
              color="#3CB371"
              title={`Sold in ${month} ${year}`}
              value={`Tk ${monthSold}`}
            />
            <StatsCard
              color="#9932CC"
              title={`Purchased - in ${month} ${year}`}
              value={`Tk ${monthPurchase}`}
            />
            <StatsCard
              color="#FF8C00"
              title={`Expense in ${month} ${year}`}
              value={`Tk ${monthExpense}`}
            />
            <StatsCard
              color="#40E0D0"
              title={`Returned in ${month} ${year}`}
              value={`Tk ${monthReturned}`}
            />
            <StatsCard
              color="#BA55D3"
              title={`Profit ${month} ${year}`}
              value={`Tk ${monthProfit}`}
            />
          </div>
        </div>
